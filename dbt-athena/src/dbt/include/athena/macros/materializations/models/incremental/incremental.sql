{% materialization incremental, adapter='athena', supported_languages=['sql', 'python'] -%}
  ...
  {% if existing_relation is none %}
    {% set query_result = safe_create_table_as(False, target_relation, compiled_code, model_language, force_batch, disable_batch_fallback, build_strategy) -%}
    {%- if model_language == 'python' -%}
  ...
  {%- elif (
    ...
    {% set query_result = safe_create_table_as(False, tmp_relation, compiled_code, model_language, force_batch, disable_batch_fallback, build_strategy) -%}
    {%- if model_language == 'python' -%}
      {% call statement('create_table', language=model_language) %}
        {{ query_result }}
      {% endcall %}
