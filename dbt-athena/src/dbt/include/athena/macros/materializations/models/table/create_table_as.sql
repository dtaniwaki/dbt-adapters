...
{% macro safe_create_table_as(temporary, relation, compiled_code, language='sql', force_batch=False, batch_fallback=True) -%}
    {%- if language != 'sql' -%}
    ...
    {%- else -%}
        ...
        {%- else -%}
          {%- set compiled_code_result = adapter.run_query_with_partitions_limit_catching(create_table_as(temporary, relation, compiled_code), batch_fallback) -%}
