...
class SparkConnectSessionPool:
    ...
    def acquire(
        ...
        Reuses an idle session with the same key if available.  Otherwise
        starts a new session if the per-key count is below ``max_sessions``.
        Waits up to ``timeout`` seconds for a slot to open.
        """
        invocation_id = key[0]
