...
class TestAthenaAdapter:
    ...
    @mock_aws
    def test_check_schema_exists(self, mock_aws_service):
        mock_aws_service.create_data_catalog()
        mock_aws_service.create_database(name="existing_schema")
        self.adapter.acquire_connection("dummy")
        assert self.adapter.check_schema_exists(DATA_CATALOG_NAME, "existing_schema") is True
        assert self.adapter.check_schema_exists(DATA_CATALOG_NAME, "non_existing_schema") is False

    @mock_aws
    def test_check_schema_exists_with_other_glue_data_catalog(self, mock_aws_service):
        data_catalog_name = "other_data_catalog"
        mock_aws_service.create_data_catalog(data_catalog_name)
        mock_aws_service.create_database(name="existing_schema")
        self.adapter.acquire_connection("dummy")
