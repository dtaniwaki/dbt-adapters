...
class TestSparkConnectSubmission:
    ...
    def _make_helper(self, parsed_model, credentials, mock_pool):
        with patch(
            "dbt.adapters.athena.python_submissions.AthenaSparkSessionManager"
        ) as MockSessionManager:
        ...
        helper.__dict__["_spark_connect_pool"] = mock_pool
        return helper
    ...
    def test_dispatches_to_spark_connect_for_engine_version_35(
        ...
        mock_pool = Mock()
        helper = self._make_helper(spark_connect_parsed_model, mock_credentials, mock_pool)
        assert helper._is_spark_connect is True

        with patch.object(
            helper, "_submit_spark_connect", return_value={"SparkConnect": True}
        ) as submit_spark_connect:
            result = helper.submit("spark.sql('SELECT 1')")

        submit_spark_connect.assert_called_once_with("spark.sql('SELECT 1')")
    ...
    def test_calculations_path_is_used_when_engine_version_not_35(
        ...
        mock_pool = Mock()
        helper = self._make_helper(calculations_parsed_model, mock_credentials, mock_pool)
    ...
    def test_successful_submission_releases_session(
        ...
        monkeypatch.setattr(
            "dbt.adapters.athena.python_submissions._create_athena_channel_builder",
            Mock(return_value=Mock()),
        )

        result = helper.submit("x = 1")
    ...
    def test_transient_error_retries_with_new_session(
        ...
        result = helper.submit("spark.run()")
    ...
    def test_session_key_varies_with_engine_config(
        ...
        mock_pool = Mock()
        helper_a = self._make_helper(spark_connect_parsed_model, mock_credentials, mock_pool)
        ...
        helper_b = self._make_helper(other_model, mock_credentials, mock_pool)

        assert helper_a._session_fingerprint != helper_b._session_fingerprint
        assert helper_a._session_key != helper_b._session_key
