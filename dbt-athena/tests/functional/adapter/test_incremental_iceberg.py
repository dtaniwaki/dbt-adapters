...
models__merge_exclude_columns_schema_change_fail_sql = """
{{ config(
    materialized = 'incremental',
    unique_key = 'id',
    incremental_strategy='merge',
    table_type='iceberg',
    merge_exclude_columns=['_is_deleted'],
...
seeds__expected_merge_exclude_columns_schema_change_fail_csv = """id,msg
1,hey
3,new
"""


class TestIcebergMergeExcludeColumnsSchemaChangeFail:
    @pytest.fixture(scope="class")
    def models(self):
        return {
            "merge_exclude_columns_schema_change.sql": models__merge_exclude_columns_schema_change_fail_sql,
        }

    @pytest.fixture(scope="class")
    def seeds(self):
        return {
            "expected_merge_exclude_columns_schema_change.csv": seeds__expected_merge_exclude_columns_schema_change_fail_csv,
        }

    def test__merge_exclude_columns_with_schema_change_fail(self, project):
        """merge_exclude_columns should be excluded from schema change detection"""

        expected_seed_name = "expected_merge_exclude_columns_schema_change"
        run_dbt(["seed", "--select", expected_seed_name, "--full-refresh"])

        relation_name = "merge_exclude_columns_schema_change"
